	...
	always_ff @(posedge soup_clk_i) begin
		if(~soup_rstn_i) begin
			...
			case (soup_cmd_st_r)
				...
				// in CNTR_LOAD we just let the counter latch in the payload_size_r byte, and then move onto RCV_PAYLOAD
				LOAD_CNTR : begin
				...
				// depending on the error flag we need to make the TX send an ACK or NACK.
